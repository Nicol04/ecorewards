                ...
                <div class="col-lg-8">
                    ...
                    <div class="pb-3">
                        <div class="blog-item">
                            <div class="position-relative">
                                ...
                                <div class="blog-date">
                                    <!-- Si necesitas un botón para ir atrás, este es el lugar -->
                    ...
                    <div class="bg-white" style="padding: 30px; margin-bottom: 30px;">
                        <h4 class="text-uppercase mb-4" style="letter-spacing: 5px;">
                            {{ !empty($comentarios) && count($comentarios) > 0 ? count($comentarios) . ' Comentarios' : '0 Comentarios' }}
                        </h4>

                        @if($recompensa->canjes->count() > 0)
                        @foreach($recompensa->canjes as $canje)
                        @if($canje->canje_comentario)
                        <div class="media mb-4">
                            <img src="{{ $canje->canje_comentario->fotoObjeto ?? 'img/default.jpg' }}" alt="Foto" class="img-fluid mr-3 mt-1" style="width: 45px;">
                            <div class="media-body">
                                <h6>{{ $canje->canje_comentario->canje->usuario->nombreUsuario ?? 'Anónimo' }}
                                    <small><i>{{ $canje->canje_comentario->fechaComentario }}</i></small>
                                </h6>
                                <p>{{ $canje->canje_comentario->comentario }}</p>
                                <p><strong>Puntuación:</strong> {{ $canje->canje_comentario->puntuacion }}/5</p>
                            </div>
                        </div>
                        @endif
                        @endforeach
...
.stars {
        display: flex;
        direction: row;
        justify-content: flex-end; /* Asegura que las estrellas estén alineadas a la derecha */
    }

    .stars i {
        font-size: 25px;
        color: #393939; /* color gris oscuro para el fondo */
        cursor: pointer;
        transition: color 0.2s ease-in-out;
    }

    .stars i.selected {
        color: #fbea08; /* color amarillo para las estrellas seleccionadas */
    }

    .stars i.hovered {
        color: #fbea08; /* color amarillo al pasar el mouse */
    }
